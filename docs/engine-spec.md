...
**Response shape:** The response is the full `user_settings` row plus a `recalculation` field. When `main_currency` actually changed, `recalculation` is `{regular_transactions, transfer_transactions, orphan_transfer_legs, inbox_items, total}` (the same summary written to the activity log) so clients can display "rewrote N transactions" without a follow-up read. When no recalc ran (empty body, unchanged `main_currency`, or any non-currency field update), `recalculation` is `null` per the null-over-omission rule — never absent. Same shape and precedent as `PUT /accounts/{account_id}/reconciliations/order`'s `recalculated_count`. The activity_log entry remains the canonical audit record; the response field is a client convenience that avoids a `GET /activity` round-trip.
