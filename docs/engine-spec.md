...
**Moves between accounts:** The engine has **no transfer concept** (removed 2026-08-10, owner decision — the auto-paired implementation was retired). Moving money between two accounts is recorded as **two ordinary rows** — an outflow on one account and an inflow on the other — categorized with ordinary user categories. There is no pairing, no netting, and no system category marking them, and monthly inflow/outflow totals **include** such moves; that is accepted, not an oversight.
