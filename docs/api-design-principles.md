...
**Offline writes — iOS only, by design.** The local replica is read-only by default. Writes require network connectivity, and clients that lose connectivity should surface an error rather than silently buffering. **The iOS client is the single planned exception:** it will layer a durable command queue on top of the replica so user actions taken offline replay automatically on reconnect — the Todoist mobile pattern. Each action is recorded as a `{type, uuid, args}` command in local SQLite, applied optimistically to the local replica for immediate UI, and POSTed to the engine in order when connectivity returns. The engine remains the conflict-resolution authority; `X-Idempotency-Key` (§4) makes replays safe, client-generated UUIDs (§2) make offline-created resources referenceable before the round trip, and tombstones in `/sync` (§5) let the replica reconcile after the queue flushes. All engine primitives needed for this are already in place — adding the queue is a per-client implementation effort, not an engine change, and its full design will live in `expense_world_ios`'s own docs.

**CLI and web are intentionally excluded.** The CLI fails fast with a network error so a developer at a terminal gets synchronous feedback rather than silent buffering. The web dashboard is read-only first (see repo split below) and a queue offers no value in a tab that won't survive a refresh. If either client's product story changes, adding offline writes is a per-client decision and a documented commitment in that client's repo, not a default they inherit.
...
### 11. IDs-Only — Achieved, Not Aspirational

Responses reference related resources **by ID only**. No endpoint hydrates a name alongside an ID — there is no `category_name` next to `category_id`, no `account_name`, no `hashtag_name` anywhere in the response schemas. Clients resolve display names from their local replica (§3b), which already holds every category, account, and hashtag.

This was originally written as a direction to move toward, with hydrated names tolerated in Phase 1 for development convenience. That convenience was never actually taken: the engine has been IDs-only since the first resource endpoints shipped. It is now a **standing rule** rather than a goal — adding a hydrated name to any response is a regression, because it introduces a second copy of a mutable value that goes stale the moment the referenced row is renamed. See `lessons-lunchmoney.md §7`.
...
User-controlled collection ordering uses a per-scope `sort_order: integer NOT NULL DEFAULT 0` column on the resource row, ordered ASC on list endpoints. Used today by `expense_bank_accounts`, `expense_categories`, `expense_hashtags`, and `expense_reconciliations`. New rows append (`max(sort_order) + 1` for the scope, or `1` if none). Soft-deleted rows keep their value but are skipped for active listings — restoring reclaims the original slot. Cross-scope `sort_order` values are meaningless and never compared (e.g., reconciliations across two different accounts have independent sequences).

**Single-row updates vs. bulk reorder.** For most resources, `sort_order` is mutable via the standard `PUT /{resource}/{id}` body — the engine accepts the new integer and writes it directly. This works because reordering one row in those collections has no side effects on its neighbors.

For resources where reordering has *cascade effects on other rows*, single-row `PUT` is unsafe — it would skip the cascade and leave the collection in an inconsistent state. These resources expose a dedicated bulk endpoint `PUT /{parent}/{id}/{children}/order` with body `{ "ordered_ids": [...] }` and reject `sort_order` in the standard `PUT` with `422 VALIDATION_ERROR`. The bulk endpoint runs the renumber + cascade in one DB transaction. Concrete instance: `PUT /accounts/{account_id}/reconciliations/order` exists because chained beginning balances must recompute when neighbors change; a single-row `sort_order` PUT would leave downstream chained rows pointing at a different upstream than their stored balance reflects.

**Subset semantics.** The bulk endpoint accepts any subset of the collection's UUIDs. The engine reuses the `sort_order` slots currently held by the submitted rows (sorted ASC) and reassigns them in the new order. Rows not in the array are untouched. This lets clients reorder a window (e.g., one calendar year of reconciliations) without sending the whole collection. The engine never imposes a "year picker" or scope object — the client expresses scope through the IDs it sends. See `lessons-todoist.md §1` (UUID-first) for the underlying pattern.
...
**The machine-readable contract.** The engine exposes an OpenAPI spec, generated by FastAPI from the route definitions and served at `/openapi.json` (Swagger UI at `/docs`). An AI agent can hand that spec to any client developer — or to itself — and produce correct matching code with no guesswork about route names, parameter types, or response shapes. *(An `llms.txt` companion has been discussed but is not implemented — do not assume one exists.)*
...
Swift / SwiftUI. Minimalist mobile interface. Has no knowledge of the database or business logic — it only knows how to ask the engine for data and display it. **Sole client expected to implement an offline write queue** (§3b) so user actions taken without signal feel instant and replay on reconnect. Built last, only if the web dashboard proves insufficient for mobile use.
...
*Last updated: 2026-07-30 (local deployment profile; §11 corrected to reflect shipped IDs-only behavior)*
