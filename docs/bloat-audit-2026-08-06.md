...
- **`_STARLETTE_CODE_MAP` entries for 413/415/429** (`errors.py:15-17`) — codes the engine cannot emit (no body limit, no media-type guard, no rate limiting). Low priority; `HTTP_ERROR` fallback already handles them.
- **`fetch_hashtag_ids_map`** (`transactions.py:80`) — module-public with exactly one caller (inside `attach_hashtag_ids`); rename to private and drop the redundant seed-vs-`setdefault` double key handling.
- **Keep:** `clear_rate_cache` (test-only callers, but the module-global cache genuinely needs a reset hook); the prose tombstone blocks (`errors.py:95-102`, `exchange_rate.py:169-183`, `config.py:16-19`, `transactions.py:27-42`) — decision records, not dead code. The "balance write IS the balance change" point is made ~5× across domain modules and could be reduced to pointers at the sql/021/022 migration headers, owner's call.
