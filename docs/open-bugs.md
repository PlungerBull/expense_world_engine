...
- **1.7-round** FX rounding split — `_fetch_rate_from_db` returns `float(row["rate"])`, truncating the stored `numeric` to binary float, after which Python's `round()` applies banker's rounding; the SQL fragments keep full `numeric` and round half-away-from-zero. At rate 3.3373 a $50.00 balance is S/166.87 in SQL and S/166.86 in Python. Bounded at one cent and it does **not** compound — `fetch_balances` sums the native balance first and converts once, so the error is one cent per account, ever. Live only on `current_balance_home_cents` and `GET /exchange-rates` (the two Python-side conversions); reports and dashboards are SQL throughout, and they answer a different question anyway (each transaction at its own date's rate vs. the whole balance at today's), so this is "a cent off true", not "two screens disagree". Fix is `Decimal` + `ROUND_HALF_UP` throughout. It is also what forces `tests/test_home_currency_parity.py` to compare rates rather than cents — that file's docstring says not to "strengthen" it into a cents comparison, and this row is the reason to revisit.
- **1.7-cache** Negative-lookup cache has no TTL distinct from the positive one — `get_rate` remembers a "no rate" answer for the same hour it remembers a real one, so a corrected or newly-backfilled rate takes up to an hour to reach the surfaces that use the Python path. Smaller than it looks: reports run in SQL and are uncached, and a missing rate for *today* resolves to yesterday's row rather than to `None`, so the negative branch is only reached below the earliest stored date. The realistic bite is the hour after a backfill, when `GET /exchange-rates` and account balances still say unconvertible for old dates that the reports can already price.
