...
- **7.1** `POST`/`PUT /inbox` do no referential or ownership validation — another user's `account_id` is stored and only rejected at promote. Use the existing `validate_active_account` / `validate_active_category`. *(Narrowed three times: 2026-08-08, the `?ready=true` EXISTS predicates gained the `user_id` arm, so a cross-tenant draft no longer shows as promotable; 2026-08-08, a MALFORMED FK now 422s at the schema boundary — 6.6; and 2026-08-11, the transfer removal deleted `transfer_account_id`, so the unvalidated surface is `account_id` + `category_id` only. What remains is the write-side acceptance of well-formed ids that are nonexistent, deleted, or another tenant's.)*
...
- **6.7** `POST`/`PUT /transactions` accept a system `category_id` — `validate_active_category` checks `deleted_at` only, not `is_system`, so a user can manually file an ordinary expense under `@Opening`, which flow reports exclude entirely: the row would move the balance while silently vanishing from every monthly report. ⚠️ The fix belongs at the **public boundary**, not inside `validate_active_category`: the one internal path must keep working — `create_opening_balance` delegates to `create_transaction` with `@Opening`, which calls that same helper. Same boundary-vs-internal shape as 7.4's reserved-name check (closed 2026-08-07). The batch path has the same hole — `create_batch`'s vectorised category query filters `deleted_at` only — so the boundary fix needs **three** call sites: create, update, batch. *(Re-scoped 2026-08-11 by the transfer removal: `@Transfer`/`@Debt` no longer exist; `docs/currency-model-decision.md`'s "system categories are engine-assigned only" section is now `@Opening`-scoped and names these two holes.)*
