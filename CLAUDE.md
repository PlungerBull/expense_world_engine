...
*Amended 2026-08-02, then 2026-08-05 (`sql/021`, the read-time currency migration). This convention previously read "every response that contains an amount must include a home-currency version", which was written for the stored-column model and made every transaction carry a PEN value no client rendered.*
...
**Collection ordering**
User-ordered collections use a per-scope `sort_order integer NOT NULL DEFAULT 0`, listed ASC, writable via the normal `PUT`. New rows append (`max+1` within the scope). Soft-deleted rows keep their slot and reclaim it on restore. Cross-scope values are meaningless and never compared. No collection currently has a cascading reorder — the one bulk-reorder route (`PUT /accounts/{id}/reconciliations/order`) went with reconciliation chaining in `sql/025`; reconciliations are ordered by their dates now, not by a user-managed slot.

**Tenant isolation is engine-side, not RLS**
RLS policies (`auth.uid() = user_id`) exist on all 14 tables and `rowsecurity` is on, but they are **inert** — the engine connects as the table owner and `FORCE ROW LEVEL SECURITY` is not set, so Postgres bypasses them. The `user_id` predicate in every query is the *only* thing isolating data. Treat a missing `user_id` filter as a security defect, not a tidiness one. Before a second user exists, either connect as a non-owner role or issue `FORCE ROW LEVEL SECURITY`.
...
- **Splits are a future migration's problem, by construction.** `parent_transaction_id` was dropped in `sql/024` (it was reserved and never non-null), so a naive `SUM` is correct today because nothing a sum must exclude can exist. The documented split rule — a parent row is a display container that does not move the balance; only its children do — survives as the predicate the splits migration must add to the balance sum, written out in `account_balance.py` and `sql/022`'s header.

*Replaced the "Balance updates are atomic" convention, 2026-08-06 (`sql/022`, the computed-balance migration). That rule existed only to protect a stored derived value; deleting the value leaves it nothing to protect. The sentence above is deliberately the same one the currency model uses — the two are one idea.*
