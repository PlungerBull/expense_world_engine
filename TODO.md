...
1. **5.5** — the three reconciliation state-machine gaps *(was four; the sibling delete warning closed 2026-08-08 with bloat-audit Tier 2 §6)*.
2. **8.2** — batch CREATE snapshots log `hashtag_ids: []` *(the transfer path's copy left with the feature, 2026-08-11)*.
3. **1.7 remainder** — dedicated FX-hygiene pass: rate plausibility vs prior day, negative-lookup cache TTL, archived-account currencies in the fetch list, `Decimal`/`ROUND_HALF_UP`.
4. **Four ⚪ lows** — `restore_category` skips the reserved-name check (7.4-r); `?hashtag_id=` filter lacks `transaction_source = 1` (hashtag-filter — or fold into the inbox-hashtags feature below, whichever ships first); inbox titles stored verbatim, whitespace-only can promote (inbox-title, found 2026-08-08 — fix wants an owner call on 422-vs-NULL); `color or` collapses an explicit empty string to the default (account-color, found 2026-08-08 — needs a reject-vs-store decision).
